...
module.exports = {
  ...
  createAppsetting: (req, res) => {
    const companyId = req.user.companyId;
    const newAppsetting = new Appsetting({ ...req.body, company: companyId });
    newAppsetting
      .save()
      .then((savedData) => {
  ...
  getAppsettingWithId: async (req, res) => {
    ...
    const companyId = req.user.companyId;
    Appsetting.findOne({ _id: id, company: companyId })
      .then((resp) => {
        if (resp) {
  ...
  // // Not providing the  companyId as appsetting Id will be unique.
  //     try {
  ...
  deleteAppsettingWithId: async (req, res) => {
    try {
      ...
      await Appsetting.findOneAndDelete({ _id: id, company: companyId });
