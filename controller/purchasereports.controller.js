...
module.exports = {
  ...
  printPurchaseSummarySupplier: async (req, res) => {
    try {
      const companyId = req.user.companyId;
      ...
      if (req.query.supplier) {
        supplier_id = req.query?.supplier?._id;
        supplier_name = req.query?.supplier?.name;
        supplier_code = req.query?.supplier?.supplier_code;
        matchedCriteria.supplier = new mongoose.Types.ObjectId(supplier_id);
      }
      ...
      const data = await Purchaseinvoicedetail.aggregate([
        // 1. Filter detail records
        {
          $match: matchedCriteria,
        },

        // 2. Join Purchaseinvoice Header
        {
          $lookup: {
        ...
        // 4. Filter based on Purchaseinvoice.invoiceDate
        {
          $match: {
            "invoice.invoiceDate": {
        ...
        // 7. Convert supplier array to object
        {
          $unwind: {
            path: "$supplier",
      ...
      // ✅ Grand Total of Net Amount for ALL Suppliers
      // =====================================
      // GRAND TOTALS OF ALL SUPPLIERS
      // =====================================
      ...
      if (requesttype === "PDF") {
        ...
        res.writeHead(200, {
          "Content-Type": "application/pdf",
          "Content-Disposition":
            "attachment; filename=purchasesummarysupplier.pdf",
        });
        ...
        doc
          .font("Helvetica-Bold")
          .fontSize(14)
          .text("Supplier Purchase Summary", 0, titleY, {
            align: "center",
          });
  ...
  printPurchaseSummaryItem: async (req, res) => {
    try {
      const companyId = req.user.companyId;
      const companyObjectId = new mongoose.Types.ObjectId(companyId);
      const companyInfo = await Company.findById(companyId).lean();
      ...
      const data = await Purchaseinvoicedetail.aggregate([
        ...
        // 2. Join Purchase Invoice Header
        {
          $lookup: {
            from: "purchaseinvoices",
            localField: "piId",
      ...
      if (requesttype === "PDF") {
        ...
        res.writeHead(200, {
          "Content-Type": "application/pdf",
          "Content-Disposition": "attachment; filename=purchasesummaryitem.pdf",
        });
        ...
        doc
          .font("Helvetica-Bold")
          .fontSize(14)
          .text("Item Purchase Summary", 0, titleY, {
            align: "center",
          });
  ...
  printPurchaseInvoiceList: async (req, res) => {
    try {
      const companyId = req.user.companyId;
      const companyObjectId = new mongoose.Types.ObjectId(companyId);
      const companyInfo = await Company.findById(companyId).lean();
      ...
      const data = await Purchaseinvoicedetail.aggregate([
        ...
        // 4. Filter based on invoice date and status
        {
          $match: {
            "invoice.invoiceDate": {
              $gte: new Date(fromDate),
        ...
        // 5. Group by Purchase Invoice ID
        {
          $group: {
            _id: "$piId",
      ...
      if (requesttype === "PDF") {
        ...
        doc
          .font("Helvetica-Bold")
          .fontSize(14)
          .text("Purchase Invoice List", 0, titleY, {
            align: "center",
          });
        ...
        const drawRow = (row, index) => {
          ...
          const values = columns.map((col) => {
            ...
            switch (col.key) {
              case "piCode":
                value = row?.invoice?.piCode || "-";
                break;
              case "invoiceDate":
                value = row?.invoice?.invoiceDate
                  ? new Date(row.invoice.invoiceDate).toLocaleDateString(
                      "en-IN",
                    )
                  : "-";
                break;
        ...
        columns.forEach((col) => {
          ...
          // Empty piCode
          else if (col.key === "piCode") {
            doc.text("", totalX + 5, y + 7, {
              width: col.width - 10,
            });
          }
          // Empty invoiceDate
          else if (col.key === "invoiceDate") {
            doc.text("", totalX + 5, y + 7, {
              width: col.width - 10,
            });
          }
