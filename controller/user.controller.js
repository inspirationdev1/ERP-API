...
module.exports = {
  getAllUsers: async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message: "Server Error in Getting All User. Try later",
      });
    }
  },
  getUserWithQuery: async (req, res) => {
    try {
      ...
      res.status(500).json({
        success: false,
        message: "Error  in fetching User  with query.",
      });
    }
  },
  ...
  getUserOwnDetails: async (req, res) => {
    ...
    User.findOne({ _id: id, company: req.user.companyId })
      .then((resp) => {
        if (resp) {
  ...
  deleteUserWithId: async (req, res) => {
    try {
      ...
      res.status(200).json({
        ...
        data: UserAfterDelete,
      });
    } catch (error) {
      console.log("Error in updateUserWithId", error);
      res.status(500).json({
        success: false,
        message: "Server Error in deleted User. Try later",
      });
    }
  },
  signOut: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ success: true, messsage: "User Signed Out  Successfully." });
    } catch (error) {
  ...
  isUserLoggedIn: async (req, res) => {
    try {
      ...
      if (token) {
        ...
        if (decoded) {
          res.status(200).json({
            success: true,
            data: decoded,
            message: "User is a logged in One",
          });
        } else {
          res
            .status(401)
      ...
      res.status(500).json({
        success: false,
        message: "Server Error in User Logged in check. Try later",
      });
    }
  },
};
