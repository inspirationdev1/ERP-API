...
const { createMenu, getAllMenu, getMenuWithQuery, getMenuWithId, updateMenuWithId, deleteMenuWithId } = require("../controller/menu.controller");

router.post("/create",authMiddleware(['COMPANY','USER']), createMenu);
router.get("/fetch-all",authMiddleware(['COMPANY','USER']),getAllMenu);
router.get("/fetch-with-query",authMiddleware(['COMPANY','USER']),getMenuWithQuery);
