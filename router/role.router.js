...
const { createRole, getAllRoles, getRoleWithQuery, getRoleWithId, updateRoleWithId, deleteRoleWithId } = require("../controller/role.controller");

router.post("/create",authMiddleware(['COMPANY','USER']), createRole);
router.get("/fetch-all",authMiddleware(['COMPANY','USER']),getAllRoles);
router.get("/fetch-with-query",authMiddleware(['COMPANY','USER','TEACHER','STUDENT','PARENT']),getRoleWithQuery);
